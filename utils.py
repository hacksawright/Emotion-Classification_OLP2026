...
def main():
    ...
    def print_length_percentiles(dataset, column_name="input_ids", percentiles=[50, 75, 90, 95, 99]):
        # 1. Lấy độ dài (số lượng token) của tất cả các câu trong dataset
        lengths = [len(x) for x in dataset[column_name]]
        
        # 2. Tính các mốc percentile
        results = np.percentile(lengths, percentiles)
        
        # 3. In ra màn hình
        print(f"--- THỐNG KÊ ĐỘ DÀI TOKEN FOR '{column_name}' ---")
        print(f"Độ dài lớn nhất (Max): {np.max(lengths)}")
        for p, val in zip(percentiles, results):
            print(f"Mốc {p}% câu có độ dài <= {int(val)} tokens")
    ...
    # Kiểm tra tập Input (Đầu vào)
    print_length_percentiles(tokenized_train, column_name="input_ids")

    print("\n" + "="*40 + "\n")

    # Kiểm tra tập Target/Labels (Đầu ra)
    print_length_percentiles(tokenized_train, column_name="labels")

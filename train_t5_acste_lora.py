...
class LoRALinear(torch.nn.Module):
    """Bọc quanh một nn.Linear đã huấn luyện sẵn: đóng băng trọng số gốc, chỉ train
    hai ma trận hạng thấp A, B. Chỉ dùng torch thuần -> không cần thư viện `peft`
    (không có trong danh sách thư viện được cài sẵn của BTC, mục 5.2)."""

    def __init__(self, base_linear: torch.nn.Linear, r: int = 8, alpha: int = 16, dropout: float = 0.05):
        super().__init__()
        self.base = base_linear
        for p in self.base.parameters():
            p.requires_grad = False  # đóng băng trọng số gốc

        in_f, out_f = base_linear.in_features, base_linear.out_features
        device = base_linear.weight.device
        dtype = base_linear.weight.dtype
        self.lora_A = torch.nn.Parameter(torch.zeros(r, in_f, device=device, dtype=dtype))
        self.lora_B = torch.nn.Parameter(torch.zeros(out_f, r, device=device, dtype=dtype))
        torch.nn.init.kaiming_uniform_(self.lora_A, a=5 ** 0.5)
        # B khởi tạo = 0 -> delta ban đầu = 0, model xuất phát giống hệt model gốc
...
def inject_lora(
    ...
    """Thay các nn.Linear có tên khớp target_modules (vd q_proj, v_proj) bằng LoRALinear.
    Gọi SAU khi model đã .to(device)/đã set dtype, để LoRA A/B tạo đúng device/dtype."""
    replaced = 0
    for module in model.modules():
        for child_name, child in list(module.named_children()):
            if isinstance(child, torch.nn.Linear) and any(k in child_name for k in target_modules):
                setattr(module, child_name, LoRALinear(child, r=r, alpha=alpha, dropout=dropout))
                replaced += 1
    if replaced == 0:
        raise ValueError(
            f"Không tìm thấy Linear layer nào khớp target_modules={target_modules}. "
            "Kiểm tra lại tên module (in model để xem tên thật, vd bằng model.named_modules())."
        )
    return model


def freeze_non_lora_params(model: torch.nn.Module) -> None:
    """Đảm bảo mọi tham số KHÔNG thuộc LoRA (lora_A/lora_B) đều bị đóng băng —
    kể cả embedding, LM head, LayerNorm... phòng trường hợp chúng chưa bị freeze."""
...
def save_lora_checkpoint(model: torch.nn.Module, save_dir: Path, lora_cfg: dict) -> None:
    """Lưu RIÊNG state_dict của các tham số LoRA (nhẹ, vài MB) + config để tái tạo,
    thay vì lưu toàn bộ model (nặng, và load lại bằng from_pretrained thường sẽ lỗi
    vì tên key đã đổi do LoRALinear bọc quanh Linear gốc)."""
    save_dir.mkdir(parents=True, exist_ok=True)
    lora_state = {k: v.detach().cpu() for k, v in model.state_dict().items() if "lora_A" in k or "lora_B" in k}
...
def load_lora_checkpoint(model: torch.nn.Module, save_dir: Path) -> dict:
    """Đọc lora_config.json (để biết r/alpha/dropout/target_modules đã dùng lúc train),
    inject_lora lại đúng cấu trúc, rồi nạp lora_weights.pt vào. Trả về lora_cfg đã đọc."""
    with open(save_dir / "lora_config.json", encoding="utf-8") as f:
    ...
    lora_state = torch.load(save_dir / "lora_weights.pt", map_location="cpu")
    missing, unexpected = model.load_state_dict(lora_state, strict=False)
    # missing sẽ chứa toàn bộ tham số base (bình thường, vì ta chỉ lưu lora_*) -> không cảnh báo về base
    unexpected_real = [k for k in unexpected if "lora_A" in k or "lora_B" in k]
    if unexpected_real:
        raise RuntimeError(f"Key LoRA không khớp khi nạp checkpoint: {unexpected_real}")
    return lora_cfg
...
def main():
    ...
    parser.add_argument(
        "--use_lora",
        action="store_true",
        help="Fine-tune tiết kiệm tham số bằng LoRA thủ công (thuần torch, không cần peft) "
        "thay vì full fine-tune. Khuyến nghị cho model lớn (vd Qwen0.6B) khi VRAM hạn chế.",
    )
    parser.add_argument("--lora_r", type=int, default=8, help="Hạng (rank) của ma trận LoRA.")
    parser.add_argument("--lora_alpha", type=int, default=16, help="Hệ số scale LoRA (thường = 2*r).")
    parser.add_argument("--lora_dropout", type=float, default=0.05)
    parser.add_argument(
        "--lora_target_modules",
        type=str,
        default="q_proj,k_proj,v_proj,o_proj",
        help="Danh sách tên Linear cần chèn LoRA, phân cách bằng dấu phẩy "
        "(Qwen: q_proj,k_proj,v_proj,o_proj; có thể thêm gate_proj,up_proj,down_proj cho MLP; "
        "T5: q,k,v,o).",
    )
    parser.add_argument(
        "--gradient_checkpointing",
        action="store_true",
        help="Bật gradient checkpointing để giảm VRAM (đánh đổi thêm ~20-30% thời gian compute).",
    )
    ...
    if args.predict_only:
        ...
        if (ckpt_dir / "lora_config.json").exists():
            # Checkpoint được train bằng LoRA: phải dựng lại model GỐC từ pretrained_src
            # (không phải từ ckpt_dir, vì ckpt_dir chỉ chứa lora_weights.pt), rồi chèn LoRA
            # với đúng cấu hình (r/alpha/dropout/target_modules) đã dùng lúc train, sau đó
            # mới nạp trọng số LoRA đã lưu.
            model_po = AutoModelForCausalLM.from_pretrained(pretrained_src, **pre_kw, trust_remote_code=True)
            model_po.to(device)
            load_lora_checkpoint(model_po, ckpt_dir)
        else:
    ...
    if args.use_lora:
        ...
        print(
            f"[LoRA] target_modules={lora_target_modules} r={args.lora_r} alpha={args.lora_alpha} "
            f"-> {trainable:,}/{total:,} tham số train được ({100 * trainable / total:.2f}%)"
        )

    if args.gradient_checkpointing:
        model.gradient_checkpointing_enable()
        model.config.use_cache = False  # bắt buộc tắt cache khi bật checkpointing, không thì lỗi/ vô nghĩa
    ...
    if args.use_lora:
        # Không dùng trainer.save_model() ở đây: nó sẽ lưu state_dict với tên key đã đổi
        # (vd "...q_proj.base.weight" thay vì "...q_proj.weight") -> AutoModelForCausalLM
        # .from_pretrained() thường sau này sẽ KHÔNG load lại đúng. Lưu riêng phần LoRA
        # (nhẹ, đủ để tái lập cùng model gốc pretrained_src) + tokenizer.
        ...
        save_lora_checkpoint(
            ...
            lora_cfg={
                ...
                "dropout": args.lora_dropout,
                "target_modules": lora_target_modules,
                "base_pretrained_src": str(pretrained_src),
            },
        )
        tokenizer.save_pretrained(str(best_dir))
        print(f"[LoRA] Đã lưu lora_weights.pt + lora_config.json tới {best_dir}")
    else:
